...
def find_template_matches(image_path,template_path,threshold,output_file):
    # 读取图片
...
def batch_find_template_matches(image_dir,template_path,threshold):
    for image_file in os.listdir(image_dir):
        if not image_file.endswith(".jpg"):
            continue
        image_path = os.path.join(image_dir,image_file)
        output_file = os.path.splitext(image_file)[0] +".txt"
        find_template_matches(image_path,template_path,threshold,output_file)
...
if __name__ == "__main__":
    ...
    batch_find_template_matches(image_dir,template_path,threshold)
